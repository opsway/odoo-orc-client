...
def migrate(cr, version):
    # 19.0.1.0.7 replaces this Many2one with the Integer `orc_api_key_ref`
    # and removes the field. Post scripts run in version order so this one
    # still sees the column on any upgrade path, but guard anyway rather
    # than break an upgrade over a column that no longer needs healing.
    cr.execute(
        """
