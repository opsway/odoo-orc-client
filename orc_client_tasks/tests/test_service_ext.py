...
class TestOrcClientTasksExt(TransactionCase):
    ...
    def test_embed_return_to_uses_the_seeded_theme(self):
        ...
        #
        # This assertion used to read `dark`. It was correct when written
        # and went stale in d63c1f5 ("light theme by default"), which
        # flipped EMBED_THEME_DEFAULT and added the seed row in one commit.
        # Note it did NOT survive on an install without the seed data:
        # `get_param` falls back to the in-source default, which is also
        # `light`, so the old assertion was red either way.
