...
class SemanticSearchContractTests(TransactionCase):
    def setUp(self):
        ...
        # Seed three pages with distinct vectors. The cron path
        ...
        self.a1 = Page.create({"name": "Aligned", "content": "<p>x</p>"})
        self.a2 = Page.create({"name": "Orthogonal", "content": "<p>y</p>"})
        self.a3 = Page.create({"name": "Opposite", "content": "<p>z</p>"})
        ...
        Embedding.create([
            {
                "model": "document.page", "res_id": self.a1.id,
            ...
            {
                "model": "document.page", "res_id": self.a2.id,
            ...
            {
                "model": "document.page", "res_id": self.a3.id,
                "vector_blob": _norm([-1.0, 0.0, 0.0, 0.0]).tobytes(),
                "content_hash": "h3",
                "indexed_at": "2026-05-07 00:00:00",
                "provider": "openai:text-embedding-3-small",
    ...
    def test_returns_refs_only(self):
        # No title, no snippet, no content — the README's permission
    ...
    def test_top_hit_is_the_aligned_page(self):
        with self._patch_provider():
            out = self.env["orc.embedding"].semantic_search(
                "anything", limit=1,
            )
        self.assertEqual(len(out), 1)
        self.assertEqual(out[0]["model"], "document.page")
